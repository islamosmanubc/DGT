...
class DGTree(object):
    ...
    def spLoad(self,name):
        ckpt = np.load(name, allow_pickle=True, encoding="latin1").tolist()
        self.currentDepth = ckpt['currentDepth']
        root = ckpt['tree']
        self.root = DecoderTreeNode()
        self.copynodes(self.root,root,None)
        self.loadNodeWeights(root = self.root)
        self.datasetMapping = ckpt['datamapping']

    def copynodes(self,n,root,parent):
        n.parameters = root.parameters
        n.tasks = root.tasks
        n.parent = parent
        n.depth = root.depth
        n.children = []
        n.trainable = root.trainable
        n.parentTasks = root.parentTasks
        n.name = root.name
        n.taskFmMap = []
        l = []
        for i in range(len(n.tasks)):
            if n.tasks[i] < 7:
                l.append(n.tasks[i])
        n.tasks = l
        for i in range(len(root.children)):
            newn = DecoderTreeNode()
            n.children.append(newn)
            self.copynodes(newn,root.children[i],n)
        
        self.nodesMapping.update({n.name:n})
