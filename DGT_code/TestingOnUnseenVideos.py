...
def getfiles(dataset,split):
    ...
    if dataset == 'SEGTRACK':
        pathf = basepath+'datasets/segtrackv2/test/JPEGImages'
        tasks = os.listdir(pathf)
        pathf = basepath+'datasets/segtrackv2/test'
        for task in tasks:
        ...
        pathf = basepath+'datasets/DAVIS/test/480p'
        tasks = os.listdir(pathf)
        pathf = basepath+'datasets/DAVIS/test'
        for task in tasks:
...
def train(X, Y,Tl,Xe,Ye,Tle,data_name):
    ...
    tree.loadPre('../weights/base_weights/Large_'data_name+'_weights_19.npy')
    tree.load('../weights/growing_tree_weights/Large_tree_weights_'+data_name+'.npy')
    tasks = len(X)
