...
def getfiles(dataset):
    ...
    if dataset == 'base':
        ...
        pathf = basepath+'datasets/segtrackv212/JPEGImages1'
        tasks = os.listdir(pathf)
        pathf = basepath+'datasets/segtrackv212'
        for task in tasks:
            Y_list.append(glob.glob(os.path.join(pathf,'GroundTruth',task, '*.png')))
            X_list.append(glob.glob(os.path.join(pathf,'JPEGImages1',task, 'input','*.png')))
        ...
        for task in tasks:
            Y_list.append(glob.glob(os.path.join(pathf,'Annotations/GT',task, '*.png')))
            X_list.append(glob.glob(os.path.join(pathf,'JPEGImages/input', task,'*.jpg')))
