...
### training function    
def train(X, Y,Tl,Xe,Ye,Tle,data_name,previous_data_name):
    ...
    tree.load('../weights/growing_tree_weights/tree_weights_'+previous_data_name+'_final.npy')
    tasks = len(X)
    
    previous_tasks = len(tree.root.tasks)+1
    ...
    f = open('../results/'+data_name+expr+'_test_before.txt','w')
    for t in range(tasks):
        ...
        if len(cx) > 5:
            nodeName,isNewNode = tree.createOrReuseNodeFM(cx[:5,:,:,:],cy[:5,:,:,:],task_id,True)
        else:
            nodeName,isNewNode = tree.createOrReuseNodeFM(cx,cy,task_id,True)
            
        Ylist = glob.glob(os.path.join('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/gt', '*.png'))
        Xlist = glob.glob(os.path.join('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/images', '*.png'))
        ...
        if not os.path.exists('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/images'):
            os.makedirs('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/images')
        if not os.path.exists('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/gt'):
            os.makedirs('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/gt')
    
        yd = cx[0,:,:,:3]
        d = np.uint8(yd)
        im = Image.fromarray(d)
        im.save('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/images/'+'t'+char+'_'+str(t)+'.png')
        ...
        im = Image.fromarray(d)
        im.save('../stored_examples/stored_examples_lll/'+data_name+expr+'/'+nodeName+'/gt/'+'t'+char+'_'+str(t)+'.png')
        ...
        #store task T weights
        tree.storeNodeWeights()
        tree.save('../weights/growing_tree_weights/tree_weights_'+data_name+'_'+str(t)+'.npy')
    ...
    for k in list_of_deleted_nodes:
        tree.deleteNode(k)
    tree.save('../weights/growing_tree_weights/tree_weights_'+data_name+'_final.npy',data_name,previous_tasks)
    ...
    np.save('../results/lll_iou_task_'+data_name+expr+'_test_before.npy', iou_task_based)
    np.save('../results/lll_fm_task_'+data_name+expr+'_test_before.npy', fm_task_based)
    
    #test all tasks after training is done
    ths = [0.2,0.4,0.6,0.8]
    epsilon = 1e-6
    iou_all = {}
    fm_all = {}
    ...
    f = open('../results/'+data_name+'_test_after.txt','a')
    for t in range(tasks):
        ...
        node = tree.findByTask(task_id,tree.root)
    ...
    np.save('../results/lll_iou_task_'+data_name+'_test_after.npy', iou_task_based)
    np.save('../results/lll_fm_task_'+data_name+'_test_after.npy', fm_task_based)
# =============================================================================
# Main 
# =============================================================================

previous_dataset = 'YOVOS'
current_dataset = 'CDNet'
tr_x,tr_y,tr_t = getfiles(current_dataset,'train')
te_x,te_y,te_t = getfiles(current_dataset,'test')
train(tr_x,tr_y,tr_t,te_x,te_y,te_t,current_dataset,previous_dataset)
gc.collect()